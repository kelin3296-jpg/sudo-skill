...
- **智能权限自动放行**：可选配置 hook，`/sudo` 激活时自动绕过权限确认
...
4. 安装完成后运行 `python ~/.claude/skills/sudo/sudo.py status`（Windows）或 `python3 ~/.claude/skills/sudo/sudo.py status`（Unix）
...
2. 安装到 `~/.claude/skills/sudo`
3. 如果已有旧目录，先备份再覆盖
4. 安装后运行 `python ~/.claude/skills/sudo/sudo.py status`（Windows）或 `python3 ~/.claude/skills/sudo/sudo.py status`（Unix）检查是否正常
...
如果你希望 `/sudo` 激活时自动绕过 Claude Code 的权限确认，可以配置 hook：
...
- `/sudo` 激活时 → hook 自动返回 `permissionDecision: "allow"`，跳过权限确认
- `/sudo` 未激活时 → 走正常的权限确认流程
- 所有操作仍然会被记录到 `/sudo history`，随时可以回滚

## 风险说明

- 这个 skill **默认不会自动修改 Claude Code 的权限设置**，需要手动配置 hook。
